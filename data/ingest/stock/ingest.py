...
from project_context import STOCK_HISTORIC_DATA_CLIENT as DATA_CLIENT
...
async def get_existing_range(conn: AsyncConnection, timeframe: TimeFrame, symbol: str) -> Tuple[Optional[datetime], Optional[datetime]]:
    """Return min and max timestamps for a given symbol in stock_candles_1d."""
    sql = f"""
...
async def upsert_bars(conn: AsyncConnection, timeframe: TimeFrame, symbol:str, bars: list):
    """Upsert a batch of bars into stock_candles_1d."""
...
async def ingest_chunk(conn: AsyncConnection, symbol: str, timeframe: TimeFrame, start: datetime, end: datetime) -> Optional[datetime]:
    """Ingest daily stock bars for a symbol into TimescaleDB."""
    ...
    for range_start, range_end in fetch_ranges:
        ...
        while current_start <= range_end:
            ...
            req = DATA_CLIENT.get_stock_bars(request)
...
async def ingest_range(symbol: str, timeframe: TimeFrame, start: datetime, end: datetime):
    """Backfills stock bars from start to end using chunked requests."""
...
async def get_assets() -> List[str]:
    """Return a list of symbols for all stock assets."""
    async with await get_conn() as conn:
        async with conn.cursor() as cur:
            await cur.execute("SELECT symbol FROM assets WHERE asset_type='stock' AND active=TRUE;")
            rows = await cur.fetchall()
            return [row[0] for row in rows]

async def ingest():
    """Top-level function to ingest all stock assets for all timeframes."""
    ingest_start_time = time.perf_counter()
    symbols = await get_assets()
    start_date = datetime.now(timezone.utc) - relativedelta(years=5)
    end_date = datetime.now(timezone.utc) - relativedelta(minutes=16) 
    ...
    logger.info(
        f"Stock ingest complete | symbols={len(symbols)} "
